mh2=function(theta,N,W,datat,R)
{
  ...
  for (m in 1:R)
  {
    ...
    ind.alpha=ifelse ((theta_prop[,6] > .01 & theta_prop[,6] < 0.99),1,0)
    vec=which(ind.alpha==0)
    theta_prop[vec,6]=theta[vec,6]
    
    #Loglkhd_j= rowSums(likelihood_m2_Gum(data=datat,para=theta,Sigma=Sigma))
    Loglkhd_j= rowSums(LogLike_m2_Gum(d=datat[,1],Y=as.matrix(datat[,2:3]),para=as.matrix(theta),Sigma=Sigma))
    Loglkhd_prop=rowSums(LogLike_m2_Gum(d=datat[,1],Y=as.matrix(datat[,2:3]),para=as.matrix(theta_prop),Sigma=Sigma))
    #Loglkhd_prop=rowSums(likelihood_m2_Gum(data=datat,para=theta_prop,Sigma=Sigma))
